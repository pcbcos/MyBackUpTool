...
def load_and_get_info(__source_url):  # 从文件中读取旧的info信息，从url中获取新的信息
    # noinspection PyShadowingNames
    with open(info_url, "rb") as f:
        old_dirs_info, old_files_info = pickle.load(f)
    dirs_info, files_info = Infogen.get_info(__source_url)
...
def prepare_action(__diff_info):
    diff_old_dirs, diff_new_dirs, diff_old_files, diff_new_files = __diff_info
    __action = []
    __copysize = 0
    for dir_ in diff_old_dirs:  # 文件夹删除
        dir_path, _ = dir_
        if not (dir_ in diff_new_dirs):
            __action.append((dir_path, "deldir"))

    for dir_ in diff_new_dirs:  # 文件夹新建
        dir_path, _ = dir_
        if not (dir_ in diff_old_dirs):
            __action.append((dir_path, "mkdir"))

    for file_ in diff_old_files:
        file_path, _ = file_
        __action.append((file_path, 'del'))

    for file_ in diff_new_files:
        file_path, _ = file_
        __action.append((file_path, "update"))
        __copysize += get_FileSize(os.path.join(source_url,file_path))
    return [__action, __copysize]
